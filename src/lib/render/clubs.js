function createFetchActivitiesHandler({ clubId, fetchClubActivities, extractList, setActivities, setIsLoading, setError }) { return async () => {
            setError('');
            if (!clubId) {
                setActivities([]);
                setIsLoading(false);
...
function createFetchClubApplicationHandler({ applicationId, fetchClubApplication, extractData, setApplication, setIsLoading, setError }) { return async () => {
            if (!applicationId) {
                setIsLoading(false);
                return;
            }
...
function createFetchClubDetailHandler({ clubId, fetchClubDetail, extractData, setClub, setIsLoading, setError }) { return async () => {
            setError('');
            if (!clubId) {
                setClub(null);
                setIsLoading(false);
...
function createOpenManageHandler({ clubId, router }) { return () => {
            if (!clubId) return;
            router.push(`/clubs/${clubId}/manage`);
        }; }

function createJoinRequestHandler({ clubId, router }) { return () => {
            if (!clubId) return;
            router.push(`/clubs/${clubId}/members`);
        }; }
function createFetchFeesHandler({ clubId, fetchClubFees, extractList, setFees, setIsLoading, setError }) { return async () => {
            if (!clubId) {
                setIsLoading(false);
                return;
            }
...
function createDebouncedSearchHandler({ searchTerm, setDebouncedSearchTerm, setCurrentPage }) { return () => {
...
function createTabChangeHandler({ router, setActiveTab, setIsStatusFilterOpen, setCurrentPage }) { return (tabId) =>
            () => {
                setActiveTab(tabId);
...
function createClubPressHandler({ router, alert }) { return (club) => {
...
function createCardPressHandler({ activeTab, router, onClubPress }) { return (club) =>
            () => {
...
function createSearchTermChangeHandler({ setSearchTerm }) { return (value) => {
            setSearchTerm(value);
        }; }

function createToggleStatusFilterHandler({ setIsStatusFilterOpen }) { return () => {
            setIsStatusFilterOpen((prev) => !prev);
        }; }

function createStatusFilterSelectHandler({ setStatusFilter, setIsStatusFilterOpen, setCurrentPage }) { return (value) =>
            () => {
...
function createMyStatusFilterHandler({ setMyClubStatusFilter, setCurrentPage }) { return (value) =>
            () => {
                setMyClubStatusFilter(value);
                setCurrentPage(1);
            }; }

function createCreateClubHandler({ router }) { return () => {
            router.push('/clubs/register');
        }; }

function createBrowseClubsHandler({ onTabChange }) { return () => {
            onTabChange('all')();
        }; }

function createPageChangeHandler({ setCurrentPage }) { return (pageNum) =>
            () => {
                setCurrentPage(pageNum);
            }; }

function createPrevPageHandler({ setCurrentPage }) { return () => {
            setCurrentPage((prev) => Math.max(1, prev - 1));
        }; }

function createNextPageHandler({ setCurrentPage, totalPages }) { return () => {
            setCurrentPage((prev) => Math.min(totalPages, prev + 1));
        }; }
function createManageSectionHandler({ clubId, router }) { return (route) =>
            () => {
                if (!clubId) return;
                router.push(`/clubs/${clubId}/${route}`);
            }; }
function createFetchMembersHandler({ clubId, fetchClubMembers, extractList, setMembers, setIsLoading, setError }) { return async () => {
            setError('');
            if (!clubId) {
                setMembers([]);
                setIsLoading(false);
...
function createFetchNoticesHandler({ clubId, fetchClubNotices, extractList, setNotices, setIsLoading, setError }) { return async () => {
            if (!clubId) {
                setIsLoading(false);
                return;
            }
...
function createFieldChangeHandler({ setFormData }) { return (field) =>
            (value) => {
                setFormData((prev) => ({ ...prev, [field]: value }));
            }; }

function createToggleRegularFeeHandler({ setFormData }) { return () => {
            setFormData((prev) => ({
                ...prev,
...
function createSelectRegularFeeCycleHandler({ setFormData }) { return (cycleId) =>
            () => {
                setFormData((prev) => ({
                    ...prev,
...
function createRegulationPressHandler({ router, clubId }) { return (regulationId) => {
            router.push(`/clubs/${clubId}/regulations/${regulationId}`);
        }; }

function createRegulationEditHandler({ router, clubId }) { return () => {
            router.push(`/clubs/${clubId}/regulations/create`);
        }; }
function createFetchStatsHandler({ clubId, fetchClubStats, extractData, setStatsData, setIsLoading, setError }) { return async () => {
            setError('');
            if (!clubId) {
                setStatsData(null);
                setIsLoading(false);
